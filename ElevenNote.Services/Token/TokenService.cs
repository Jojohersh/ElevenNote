...
namespace ElevenNote.Services.Token
{
    public class TokenService : ITokenService
    {
        ...
        {
            var foundUser = await _context.Users.FirstOrDefaultAsync(user => user.Username.ToLower() == model.Username.ToLower());
            
            if (foundUser is null) return null;

            var passwordHasher = new PasswordHasher<UserEntity>();

            var verifyPasswordResult = passwordHasher.VerifyHashedPassword(foundUser, foundUser.Password, model.Password);

            if (verifyPasswordResult == PasswordVerificationResult.Failed) return null;
